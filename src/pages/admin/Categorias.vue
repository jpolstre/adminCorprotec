<template>
	<q-page padding  class="q-mt-none q-pt-none">
		<q-breadcrumbs class="q-pa-xs text-grey-8">
			<q-breadcrumbs-el icon="important_devices" label="Categorias" to="/admin/categorias" class="active-breadcrumb"/>
		</q-breadcrumbs>
		...
		<draggable class="row q-mb-xl" v-model="items"  handle=".handle">
			<div class="q-pa-xs col-xl-3 col-lg-3 col-md-3 col-sm-4 col-xs-12"  v-for="item, index in items" :key="item.id">
				<q-card>
					<q-img :id="`img-categoria-${index}`" :src="item.imagePath" style="height:170.7px;">
								 ...
								 <q-btn color="primary" round  icon="more_vert">
									 <q-menu cover auto-close>
										 <q-list>
											 ...
											 <q-item clickable>
												 <q-item-section @click="eliminar(item)">Eliminar Categoria</q-item-section>
											 </q-item>
											<!--  <q-item clickable>
	...
	export default{
		...
		methods:{
			...
			guardar(){
				...
				this.$axios({
					url: `${self.$store.state.corprotec.baseUrl}guardarData/categoria`,
					method: 'post',
					data:sendFormData,
					// data: sendFormData,
			...
			eliminar(item){
				...
				if(item.id>1000000){
					...
					self.$q.loading.show()
					self.$axios.get(`${self.$store.state.corprotec.baseUrl}categorias/delData/${item.id}`)
					.then(r=>{
						console.log(r.data)
						self.$q.loading.hide()
						if(r.data.success){
			...
			changeValueInput(){
				...
				document.getElementById('img-categoria-'+this.itemElegido.index).childNodes[1].style.backgroundImage=`url(${window.URL.createObjectURL(file)})`
			...
			getData(){
				...
				.then(r=>{
					...
					self.items.forEach((item, index)=>{
						item.imagePath = `${self.$store.state.corprotec.baseUrl}public/img/img-categories/${item.imagen}?t=${Math.random()}`
						self.$q.loading.hide()
					})
